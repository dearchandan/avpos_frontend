<template>
    <div class="row align-items-center justify-content-between">
        ...
        <div class="col-auto">
            <button
                    type="button"
                    class="btn btn-link"
                    @click="download_sample"
                    :disabled="form.is_sample_processing"
                >
                <clip-loader v-if="form.is_sample_processing" loading="loading" :color="'white'" :size="'10px'" class="pt-1 pe-1">  </clip-loader>
                <span class="pt-0" v-if="form.is_sample_processing">
                    {{ $t('Downloading...') }}
                </span>
    ...
    <div class="card">
        ...
        <div class="alert mt-3" :class="'alert-'+form.custom_message.type" v-if="form.custom_message.msg != ''"> 
            <!-- <Icon icon="material-symbols:check-circle-outline" class="fs-5" />  -->
            {{ form.custom_message.msg }} 
        </div>
        <AsyncServerMessage class="py-3 mt-3" v-if="form.server_messages.messages != '' && form.custom_message.msg == ''" :server_messages="form.server_messages"  />
...
async function submit(e){
    ...
    await Swal.fire({
        ...
        title: 'Warning!',
        text: 'Are you sure about import the Inventory?',
        icon: 'warning',
    ...
    if(is_confirmed){
        ...
        await axios.post('/api/import/inventory',form_data, {
          headers: {
...
async function download_sample(){
  form.is_sample_processing = true;

  await axios.post('/api/import/inventory_sample').then( (response) => {

    if(response.data.status_code == 200) {

      form.is_sample_processing = false;
      const link = document.createElement('a');
      link.target='_blank';
