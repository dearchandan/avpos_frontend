            ...
            >
                <div class="col-12 py-3">
                    <div class="card">
                        <div class="row mb-3">
                            <div class="col-4">
                                <label class="label"> {{ $t('Authorization Access Token') }}<span class="text-danger">*</span></label>
                            </div>
                            <div class="col-8">
                              <VTextInput
                                  type="text"
                        ...
                        <div class="row mb-3">
                          <div class="col-4">
                            <label class="label"> {{ $t('Entity ID (MADA)') }}<span class="text-danger">*</span></label>
                          </div>
                          <div class="col-8">
                            <VTextInput
                                type="text"
                                name="mada_entity_id"
                                v-model="form.mada_entity_id"
                                label=""
                                :required="true"
                                :islabel=false
                                size="small"
                        ...
                        <div class="row mb-3">
                          <div class="col-4">
                            <label class="label"> {{ $t('Entity ID (Visa, MasterCard)') }}<span class="text-danger">*</span></label>
                          </div>
                          <div class="col-8">
                            <VTextInput
                                type="text"
                                name="other_entity_id"
                                v-model="form.other_entity_id"
                                label=""
                                :required="true"
                                :islabel=false
                                size="small"
...
const initialState = {
  ...
  other_entity_id:"",
  sms_template:"",
  constant_name:"HYPERPAY"
};

const form = reactive({ ...initialState });

const active_payment_gateway = toRef(props,'active_payment_gateway');
if(active_payment_gateway.value.name=='HYPERPAY'){
  form.merchant_key = active_payment_gateway.value.key;
  form.mada_entity_id = active_payment_gateway.value.mada_entity_id;
  form.other_entity_id = active_payment_gateway.value.other_entity_id;
...
const schema = yup.object().shape({
  ...
  mada_entity_id: yup.string().required(t('name is required', { name: t('Entity ID (MADA)') })),
  other_entity_id: yup.string().required(t('name is required', { name: t('Entity ID (Visa, MasterCard)') })),
...
async function savePaymentGatewaySetting(){
  ...
  form_data.append('mada_entity_id',form.mada_entity_id);
  form_data.append('other_entity_id',form.other_entity_id);
