<template>
   <div class="modal modal-lg hide" id="addPaymentMethodModal"  tabindex="-1" aria-labelledby="Add Payment Method" aria-modal="true" role="dialog" data-bs-backdrop="static" data-bs-keyboard="false" >
      <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" v-if="form.slack == '' || form.slack==undefined">{{ $t('Add Payment Method') }}</h5>
                  <h5 class="modal-title" v-else>{{ $t('Update Payment Method') }}</h5>
              ...
              <div class="modal-body">
                  ...
                  >
                    <div class="row">
                      ...
                      <div class="col-md-6">
                        <div class="form-field mt-4 pt-3">
...
<script setup>
    ...
    const schema = yup.object({
        payment_method_name: yup.string().required(t('name is required', { name: t('Payment Method Name') })).min(3,t('name must be at least n characters', { name: t('Payment Method Name'), number: 3 })),
        status: yup.bool().label('Status'),
    });
    
    

    const emit = defineEmits(['closeAddPaymentMethodModal'])

    const props = defineProps({
        show_add_payment_method_modal: Boolean,
        edit_data: {
            type: [Array,Object],
        },
    });
    ...
    function closeModal(){
        document.getElementById('frmPaymentMethod').reset();
        resetData();
        document.getElementById('close_btn').click();
        emit('closeAddPaymentMethodModal');
  ...
  watch(() => 
        props.edit_data,(value) => {
            if(value){
                ...
                form.payment_method_name = value.name;
    ...
    async function savePaymentMethod(){
        ...
        form_data.append('payment_method_name',form.payment_method_name);
        form_data.append('status',form.status);
        form_data.append('slack',form.slack);
        var submit_url = (form.slack!='' && form.slack!=undefined)?'/api/payment_method/update':'/api/payment_method/save';
