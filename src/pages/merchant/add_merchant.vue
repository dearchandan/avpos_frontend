              ...
              <div class="col-md-12 mb-4">
                <div class="card">
                  <div class="row">
                    ...
                    <div class="col-md-3" v-if="!isThirdParty">
                      <VMultiSelect
                        name="referral_account"
                        :label="labels.referral_account"
                        :placeholder="labels.referral_account"
                        :text="'value'"
                        :value="'key'"
                        :options="referral_accounts"
                        v-model="form.referral_account"
                        mode="single"
                        :selected="form.referral_account"
                      />
                    </div>
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-3">
                      <VTextInput
                        ...
                        :label="labelslabels.user_subscription_start_date"
                        :readonly="true"
                        :disabled="true"
                        :title="$t('Read Only Field')"
                      />
                    ...
                    <div class="col-md-3">
                      <VTextInput
                        ...
                        :label="
                          labels.user_subscription_active_subscriptions_at
                        "
                        :readonly="true"
                        :disabled="true"
                        :title="$t('Read Only Field')"
...
const labels = ref({
  ...
  country: t("Country"),
  referral_account: t("Referral Account"),
});
const initialState = {
  server_messages: {
    type: "",
  ...
  referral_account: isThirdParty ? authStore.user.id : "",
};
...
const schema = yup.object({
  ...
  phone: yup
    .string()
    .matches(/^[0-9]{9,10}$/, t("Phone must be 9 or 10 digits"))
    .typeError(t("must be a number", { name: t("Phone") }))
    .required(t("phone is required", { name: t("Phone") }))
    .min(9)
    .max(10),
});
...
onMounted(() => {
  getCountries();
  if (slack != "" && slack != undefined) getMerchant();
  fetchBusinessTypes();
...
const referral_accounts = ref([]);
async function fetchReferralAccounts() {
  let data = await axios.get("api/sub_admins?dropdown=1");
  referral_accounts.value = data.data;
}
  ...
  await axios
    ...
    .then((response) => {
      ...
      if (response.data.data) {
        ...
        form.referral_account = response.data.data.referral_account;
      }
...
async function saveMerchant() {
  ...
  form_data.append("referral_account", form.referral_account);

  var submit_url =
    form.merchant_slack != "" && form.merchant_slack != undefined
      ? "/api/merchant/update"
      : "/api/web_register";
