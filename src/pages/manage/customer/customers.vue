        ...
        <div class="content">
            <div class="page-heading">
              ...
              <div class="row align-items-center justify-content-between mb-3">
                <div class="col-auto">
                    <h2 class="mb-0">{{ $t('Customers') }}</h2>
                </div>
                <div class="col-auto" v-if="permissions.write">
                  <router-link class="btn btn-sm btn-light-primary2" to="/customer/add" >+ {{ $t('New Customer') }}</router-link>
                </div>
              </div>
                    ...
                    <div class="row align-items-end justify-content-between g-3">
                        <div class="col-12 col-md-auto">
                          <div class="form-field">
                            ...
                            @update:model-value="filterList()" input-class-name="form-control form-control-solid form-control-sm"/>
                          </div>
                        </div>
                        ...
                        <div class="col col-md-auto">
                          <div class="form-field">
                            ...
                            <select class="form-control form-control-sm form-control-solid"
                                    v-model="filter.type" @change="filterList">
                              <option value="">{{ $t('All') }}</option>
                              <option value="1">{{ $t('Regular') }}</option>
                              <option value="2">{{ $t('Walk-In') }}</option>
                            </select>
                          </div>
                        </div>
            ...
            <div v-else class="table-responsive">
                <table class="table table-list-data table-hover">
                    <thead class="bg-light">
                        <tr>
                            <th scope="col" width="20%">{{ $t('Customer Name') }}</th>
                    ...
                    <tbody> 
                        <tr v-if="customers != ''" v-for="(customer,branch_index) in customers" :key="branch_index" >
                            <td @click="viewCustomer(customer.slack)" role="button">
                                <div>
                                    <span class="fw-bold highlighted-list-item">{{  customer.name  }}</span>
                                    <br>
                                    <small class="text-muted">{{ $t('Code') }}: {{ customer.code }}</small>
                                </div>
                            </td>
                            <td @click="viewCustomer(customer.slack)" role="button">{{ customer.email }}</td>
                            <td @click="viewCustomer(customer.slack)" role="button">{{ customer.phone }}</td>
                            <td @click="viewCustomer(customer.slack)" role="button">{{ customer.vat_number }}</td>
                            <td @click="viewCustomer(customer.slack)" role="button"> {{ customer.gst_number }} </td>
                            <td @click="viewCustomer(customer.slack)" role="button">{{ customer.created_at_label }}</td>
                            <td>
                                <div class="form-feild">
                                    <div class="form-check form-switch form-switch-2">
                                        <input class="form-check-input" type="checkbox" role="switch" id="Status_id" @change="changeStatus(customer.slack)" :checked="customer.status == 1">
                                    </div>
                                </div>
                            </td>
                            <td class="text-end" v-if="permissions.write || permissions.delete">
                                <ActionButtonGroup :permissions="permissions" @edit="editCustomer" @delete="deleteCustomer" :slack="customer.slack" />
                            </td>
                        </tr>
                        <tr v-else>
            ...
            <div class="data-footer">
                <div class="row align-items-center justify-content-between">
                    ...
                    <div class="col-auto">                      
                        <Bootstrap5Pagination :data="pagination" :limit="5"  @pagination-change-page="getCustomers" />
                    </div>
                </div>
            </div>
        </div>
...
async function getCustomers(page = 1){
    ...
    await axios.get('/api/v2/customers/paginate', { params : {
            page : page,
...
async function changeStatus(slack){
    ...
    await axios.post('/api/customer/update_status', form_data).then( (response) => {
...
function viewCustomer(slack){
    router.push('/customer/'+slack);
}
function editCustomer(slack){
    router.push('/customer/edit/'+slack);
}
async function deleteCustomer(slack){

  var is_confirmed = false;

  await Swal.fire({
    title: t('Do you want to delete this Customer?'),
  ...
  if(is_confirmed){
    ...
    await axios.post('/api/customer/delete',{
      slack : slack
    }).then( (response) => {
