        ...
        <div class="row align-items-center">
          <div class="col-auto">
            <div class="header-back-link">
              <router-link class="text-primary" to="/users"
          ...
          <div class="col-auto">
            ...
            <h5 class="fw-bold" v-else>{{ $t("Edit User") }}</h5>
          </div>
        </div>
      </div>
    </template>
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-3">
                      <VTextInput
                        ...
                        :label="labels.email"
                        :required="true"
                        :readonly="slack != undefined && slack != ''"
                      />
                    </div>
                    ...
                    <div class="col-md-3">
                      ...
                      <VTextInput
                        ...
                        :label="
                          t('Password (Can be updated from Reset Password)')
                        "
                        :readonly="true"
                        v-else
                      />
...
watch(() => {
  if (!_.isUndefined(slack)) {
    schema = yup.object({
      ...
      status: yup
        .string()
        .required(t("name is required", { name: t("Status") })),
      phone: yup.string().matches(/^(?:[0-9]{9,10})?$/, t('Mobile number must be number and should be 9 or 10 digits')),
    });
  }
});
...
async function getUser() {
  ...
  if (response.data.data) {
    ...
    form.date_of_anniversary = response.data.data.anniversary_date;
    form.date_of_birth = response.data.data.birth_date;
    form.phone = response.data.data.phone>0?response.data.data.phone:"";
