        ...
        <template v-slot:subheader>
            <div class="header-nav-wrap">
                <div class="row align-items-center">
                    <div class="col-auto">
                        <h5 class="fw-bold">{{ $t('Sub Admins') }}</h5>
                    </div>
                </div>
            </div>
        </template>
        <div class="content">
                    ...
                    <div class="col-auto">
                        <div class="row align-items-center">
                            <div class="col-auto" v-if="permissions.write">
                                <router-link class="btn btn-sm btn-light-primary" to="/admin/user/add" >+ {{ $t('New Sub Admin') }}</router-link>
                            </div>
                        </div>
                    </div>
                  <div class="col-auto">
                              ...
                              <div class="row gx-3">
                                <div class="col-12">
                                  <div class="form-field">
                                    ...
                                    <VueDatePicker range v-model="filter.created_date" :format="'dd-MM-yyyy'"
            ...
            <div v-else class="table-responsive">
                <table class="table table-list-data table-hover">
                    <thead class="bg-light">
                        <tr>
                            <th scope="col" width="15%">{{ $t('Sub Admin Name') }}</th>
                    ...
                    <tbody> 
                        <tr v-if="users != ''" v-for="(user,branch_index) in users" :key="branch_index" >
                            <td @click="viewUser(user.slack)" role="button">
                              <span class="fw-bold highlighted-list-item">{{  user.name  }}</span>
                            </td>
                            <td @click="viewUser(user.slack)" role="button">{{ user.phone }}</td>
                             
                            <td @click="viewUser(user.slack)" role="button">{{ user.created_at_label }}</td>
                            <!-- <td>
                            ...
                            <td class="text-end" >
                                <ActionButtonGroup :permissions="permissions" @edit="editUser" @delete="deleteSubAdmin" :slack="user.slack" />
                            </td>
                        </tr>
                        <tr v-else>
            ...
            <div class="data-footer">
                <div class="row align-items-center justify-content-between">
                    ...
                    <div class="col-auto">                      
                        <Bootstrap5Pagination :data="pagination" :limit="5"  @pagination-change-page="getSubAdmins" />
                    </div>
                </div>
            </div>
        </div>
...
async function getSubAdmins(page = 1){

  form.is_listing = true;

  await axios.get('/api/sub_admins', { params : {
          page : page,
...
async function changeStatus(slack){
    ...
    await axios.post('/api/sub_admin/update_status', form_data).then( (response) => {
...
function viewUser(slack){
    //router.push('/admin/user/'+slack);
}
function editUser(slack){
    router.push('/admin/user/edit/'+slack);
}

async function deleteSubAdmin(slack){

  var is_confirmed = false;

  await Swal.fire({
    title: t('Do you want to delete this Sub Admin?'),
  ...
  if(is_confirmed){
    ...
    await axios.post('/api/sub_admin/delete',{
      slack : slack
    }).then( (response) => {
