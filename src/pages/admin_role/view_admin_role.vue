        ...
        <div class="row align-items-center">
          <div class="col-auto">
            <div class="header-back-link">
              <router-link class="text-primary" to="/admin/roles"
...
function menu_checker(payload) {
  if (payload.event.target.checked == true) {
    ...
    if (payload.type == "ACTIONS") {
      ...
      var sibling_count = 0;
      if( !_.isUndefined(payload.siblings)){
    ...
    if (payload.type == "MAIN_MENU") {
      ...
      // console.log(payload.children+ '---'+payload.children.length)
      menu_selected.value = array_remove(
        menu_selected.value,
        payload?.current
      );
...
async function savePermissions() {
  ...
  await axios
    .post("/api/admin/role/update_permission", form_data)
    .then((response) => {
