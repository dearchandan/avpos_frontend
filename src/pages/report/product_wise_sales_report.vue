        ...
        <template v-slot:subheader>
            <div class="header-nav-wrap">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto">
                        <h5 class="fw-bold">{{ $t('Product Wise Sales Report') }}</h5>
                    </div>
                </div>
            </div>
        </template>
                        ...
                        <table class="table table-list-data table-hover">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th scope="col" class="text-end">{{ $t('Returned Tax') }}</th>
                            ...
                            <tbody> 
                                ...
                                <tr v-if="report_data != ''" v-for="(report,index) in report_data" :key="index" >
                                  ...
                                  <td class="text-end">{{ getRoundedValue(report.tax_amount) }} {{ currency_symbol }}</td>
                                  <td class="text-end">{{ getRoundedValue(report.returned_tax_amount) }} {{ currency_symbol }} </td>
                                    <td class="text-end">{{ report.net_sale_qty }}</td>
                                    <td class="text-end">{{ getRoundedValue(report.net_sale_amount) }} {{ currency_symbol }} </td>
                                </tr>

                                <tr v-else>
                                    <td class="text-center py-3" colspan="12">{{ $t('No Data Found') }}</td>
                                </tr>
                            ...
                            <tfoot v-if="grand_total != '' && grand_total != null">
                                <tr>
                                    ...
                                    <th class="text-end">{{ getRoundedValue(grand_total.tax_amount) }} {{ currency_symbol }}</th>
                                    <th class="text-end">{{ getRoundedValue(grand_total.returned_tax_amount) }} {{ currency_symbol }}</th>
...
async function generateReport(page = 1){
    ...
    await axios.get('/api/product_wise_sales_report', { params : {
            page : page,
...
const grand_total = computed(() => {
    ...
    if(report_data.value.length){
        ...
        let data = {
            ...
            tax_amount : report_data.value.reduce(function (a, b) {return a + parseFloat(b.tax_amount) }, 0).toFixed(2),
            returned_tax_amount : report_data.value.reduce(function (a, b) {return a + parseFloat(b.returned_tax_amount) }, 0).toFixed(2),
...
async function exportExcel(){
    ...
    filter.dates[0] = moment(filter.dates[0]).format('YYYY-MM-DD');
    filter.dates[1] = moment(filter.dates[1]).format('YYYY-MM-DD');
    await axios.get('/api/product_wise_sales_report/export_excel', { params : {
            filter: filter,
    }}).then( (response) => {
