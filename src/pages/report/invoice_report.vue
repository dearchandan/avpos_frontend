        ...
        <template v-slot:subheader>
            <div class="header-nav-wrap">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto">
                        <h5 class="fw-bold">{{ $t('Invoice Report') }}</h5>
                    </div>
                </div>
            </div>
        </template>
            ...
            <div class="card">
                ...
                <div class="row field-m-0 align-items-end g-3">
                    ...
                    <div class="col-md-4 col-lg-3">
                      <VMultiSelect
                          name="status"
                          :label="select_status_label"
                          :placeholder="select_status_placeholder"
                          :text="'value'"
                          :value="'key'"
                          :options="status"
                          v-model="filter.selected_status"
                          mode="single"
                      />
                    </div>
                        ...
                        <table class="table table-list-data table-hover">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th scope="col">{{ $t('Invoice No.') }}</th>
                                    <th scope="col">{{ $t('Reference No.') }}</th>
                                    <th scope="col">{{ $t('Invoice Date') }}</th>
                                    <th scope="col">{{ $t('Invoice Due Date') }}</th>
                                    ...
                                    <!-- <th scope="col" class="text-end">{{ $t('After Discount') }}</th> -->
                            ...
                            <tbody> 
                                ...
                                <tr v-if="report_data != ''" v-for="(report,index) in report_data" :key="index" >
                                    ...
                                    <td class="text-center">{{ report.reference_no }}</td>
                                    <td class="text-center">{{ report.invoice_date }}</td>
                                    <td class="text-center">{{ report.due_date }}</td>
                                    <td class="text-center">{{ report.customer_name }}</td>
                                    <td class="text-center">{{ report.customer_phone }}</td>
                                    <td class="text-center">
                                      <span v-if="report.status==0" class="badge badge-primary">{{ $t('Draft') }}</span>
                                      <span v-if="report.status==1" class="badge badge-success">{{ $t('Paid') }}</span>
                                      <span v-if="report.status==2" class="badge badge-warning">{{ $t('Partial Paid') }}</span>
                                      <span v-if="report.status==3 && report.returned_status == 0" class="badge badge-warning">{{ $t('Partial Returned') }}</span>
                                      <span v-if="report.status==3 && report.returned_status == 1" class="badge badge-danger">{{ $t('Returned') }}</span>
                                    </td>

                                    <td class="text-end">{{ getRoundedValue(report.grand_subtotal_after_product_quantity) }} {{ currency_symbol }}</td>
                                    <td class="text-end">{{ getRoundedValue(report.invoice_discount) }} {{ currency_symbol }}</td>
                                    <!-- <td class="text-end">{{ getRoundedValue(report.grand_subtotal_after_product_discount - report.invoice_discount) }} {{ currency_symbol }}</td> -->
                                    <td class="text-end">{{ getRoundedValue(report.grand_product_tax) }} {{ currency_symbol }}</td>
                                    <td class="text-end">{{ getRoundedValue(report.invoice_additional_charges) }} {{ currency_symbol }}</td>
                                    <td class="text-end">{{ getRoundedValue(report.grand_product_amount) }} {{ currency_symbol }} </td>
                                </tr>

                                <tr v-else>
                                    <td class="text-center py-3" colspan="14">{{ $t('No Data Found') }}</td>
                                </tr>
                            ...
                            <tfoot v-if="grand_total != '' && grand_total != null">
                                <tr>
                                    ...
                                    <th class="text-end">{{ getRoundedValue(grand_total.invoice_discount) }} {{ currency_symbol }}</th>
                                    <!-- <th class="text-end">{{ getRoundedValue(grand_total.grand_subtotal_after_product_discount) }} {{ currency_symbol }}</th> -->
                                    <th class="text-end">{{ getRoundedValue(grand_total.grand_product_tax) }} {{ currency_symbol }}</th>
                                    <th class="text-end">{{ grand_total.invoice_additional_charges }}</th>
...
const select_status_label = t('Select Status');
const select_status_placeholder = t('All Status');
...
async function generateReport(page = 1){
    ...
    await axios.get('/api/invoice_report', { params : {
            page : page,
...
const grand_total = computed(() => {
    ...
    if(report_data.value.length){
        ...
        let data = {
            ...
            invoice_discount : report_data.value.reduce(function (a, b) {return a + parseFloat(b.invoice_discount) }, 0).toFixed(2),
            grand_subtotal_after_product_discount : report_data.value.reduce(function (a, b) {return a + parseFloat(b.grand_subtotal_after_product_discount) }, 0).toFixed(2),
            grand_product_tax : report_data.value.reduce(function (a, b) {return a + parseFloat(b.grand_product_tax) }, 0).toFixed(2),
            invoice_additional_charges : report_data.value.reduce(function (a, b) {return a + parseFloat(b.invoice_additional_charges) }, 0).toFixed(2),
...
async function exportExcel(){
    ...
    filter.dates[0] = moment(filter.dates[0]).format('YYYY-MM-DD');
    filter.dates[1] = moment(filter.dates[1]).format('YYYY-MM-DD');
    await axios.get('/api/invoice_report/export_excel', { params : {
            filter: filter,
    }}).then( (response) => {
