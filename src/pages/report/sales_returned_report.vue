        ...
        <template v-slot:subheader>
            <div class="header-nav-wrap">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto">
                        <h5 class="fw-bold">{{ $t('Sales Returned Report') }}</h5>
                    </div>
                </div>
            </div>
        </template>
                        ...
                        <table class="table table-list-data table-hover">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th scope="col">{{ $t('Return Order No.') }}</th>
                                    <th scope="col">{{ $t('Sale Return Date') }}</th>
                                    <th scope="col">{{ $t('Customer Name') }}</th>
                                    <th scope="col">{{ $t('Customer Phone') }}</th>
                                    <th scope="col">{{ $t('Payment Type') }}</th>
                                    <th scope="col">{{ $t('Return Reason') }}</th>
                            ...
                            <tbody> 
                                ...
                                <tr v-if="report_data != ''" v-for="(report,index) in report_data" :key="index" >
                                    ...
                                    <td>{{ report.return_order_number }}</td>
                                    <td>{{ report.return_date }}</td>
                                    <td>{{ report.customer_name }}</td>
                                    <td>{{ report.customer_phone }}</td>
                                    <td>{{ report.payment_method_name }}</td>
                                    <td>{{ report.reason }}</td>
                            ...
                            <tfoot v-if="grand_total != '' && grand_total != null">
                                <tr>
                                    ...
                                    <th class="text-end">{{ getRoundedValue(grand_total.returned_tax_amount) }}</th>
...
async function generateReport(page = 1){
    ...
    await axios.get('/api/sales_returned_report', { params : {
            page : page,
...
const grand_total = computed(() => {
    ...
    if(report_data.value.length){
        ...
        let data = {
            returned_discount_amount : report_data.value.reduce(function (a, b) {return a + parseFloat(b.order_discount) }, 0).toFixed(2),
            returned_tax_amount : report_data.value.reduce(function (a, b) {return a + parseFloat(b.tax) }, 0).toFixed(2),
          returned_amount : report_data.value.reduce(function (a, b) {return a + parseFloat(b.total) }, 0).toFixed(2),
        };
...
async function exportExcel(){
    ...
    filter.dates[0] = moment(filter.dates[0]).format('YYYY-MM-DD');
    filter.dates[1] = moment(filter.dates[1]).format('YYYY-MM-DD');
    await axios.get('/api/sales_returned_report/export_excel', { params : {
            filter: filter,
    }}).then( (response) => {
