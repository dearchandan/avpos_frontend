        ...
        <div class="row align-items-center">
          <div class="col-auto">
            <div class="header-back-link">
              <router-link class="text-primary" to="/vouchers"
              ><Icon icon="fa6-solid:chevron-left" />
                {{ $t("Vouchers") }}</router-link
              >
            </div>
          </div>
          <div class="col-auto">
            ...
            <h5 class="fw-bold" v-else>{{ $t("Edit Voucher") }}</h5>
          </div>
        </div>
      </div>
    </template>
              ...
              <div class="col-md-12">
                <div class="card">
                  <div class="row">
                    ...
                    <div class="col-md-2">
                      <VMultiSelect
                          :label="labels.closing_voucher_user"
                          name="closing_voucher_user_id"
                          :text="'name'"
                          :value="'id'"
                          :options="voucher_users"
                          mode="single"
                          v-model="form.closing_voucher_user_id"
                          :placeholder="labels.select_voucher_user"
                          :required="true"
                      />
                    </div>
                    <div class="col-md-2">
...
const labels = ref({
  voucher_number: t("Voucher Number"),
  voucher_date: t("Voucher Date"),
  balance_amount: t("Balance Amount"),
  withdrawal_amount: t("Withdrawal Amount"),
  note: t("Note"),
  closing_voucher_user: t("Closing Voucher User"),
  select_voucher_user: t("Select User"),
...
let schema = yup.object({
  voucher_number: yup.string().required(t("name is required", { name: t("Voucher Number") })),
  voucher_date: yup.date().required(t("name is required", { name: t("Voucher Date") })),
  closing_voucher_user_id: yup.string().required(t("name is required", { name: t("Voucher User") })),
...
async function getAddData() {
  await axios
      .get("/api/voucher/add", { params : {type:1} })
      .then((response) => {
        if (response.data.status_code == 200) {
          ...
          _.forEach(response.data.data.voucher_users, (item) => {
            if(item.status == 1){
              ...
              voucher_users.value.push(template);
            }
          })
        } else {
          try {
...
async function getVoucher() {
  ...
  if (response.data.data) {
    ...
    form.voucher_slack = response.data.data.slack;
    form.voucher_number = response.data.data.voucher_number;
...
async function saveVoucher() {
  ...
  form_data.append("slack", form.voucher_slack);
  form_data.append("type", 1);
  var submit_url =
      form.voucher_slack != "" && form.voucher_slack != undefined
          ? "/api/voucher/update"
          : "/api/voucher/save";
