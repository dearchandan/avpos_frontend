    ...
    <div class="content">
      <div class="page-heading">
        ...
        <div class="row align-items-center justify-content-between mb-3">
          <div class="col-auto">
            <h2 class="mb-0">{{ $t('Vouchers') }}</h2>
          </div>
          <div class="col-auto" v-if="!authStore.isMerchant">
            <router-link class="btn btn-sm btn-light-primary2" to="/voucher/add" >+ {{ $t('New Voucher') }}</router-link>
          </div>
        </div>
      ...
      <div v-else class="table-responsive">
        <table class="table table-list-data table-hover">
          ...
          <tr>
            ...
            <th scope="col" width="10%">{{ $t('Voucher No.') }}</th>
            ...
            <th scope="col" width="10%">{{ $t('Voucher Date') }}</th>
            <th scope="col" width="10%">{{ $t('Payment Status') }}</th>
            <th scope="col" class="text-end" width="5%" v-if="!authStore.isMerchant">{{ $t('Action') }}</th>
          </tr>
          </thead>
          <tbody>
          <tr v-if="vouchers != ''" v-for="(voucher,branch_index) in vouchers" :key="branch_index" >
            <td @click="viewVoucher(voucher.slack)" role="button">{{ voucher.voucher_number }}</td>
            <td @click="viewVoucher(voucher.slack)" role="button">{{ voucher.withdrawal_amount }}</td>
            <td @click="viewVoucher(voucher.slack)" role="button" v-if="authStore.isMerchant">{{ voucher.branch.name }}</td>
            <td @click="viewVoucher(voucher.slack)" role="button">{{ voucher.voucher_user.name }}</td>
            <td @click="viewVoucher(voucher.slack)" role="button">{{ voucher.voucher_date_label }}</td>
            <td @click="viewVoucher(voucher.slack)" role="button">
            ...
            <td class="text-end" v-if="!authStore.isMerchant">
              <ActionButtonGroup :permissions="permissions" @edit="editVoucher" :slack="voucher.slack" v-if="voucher.status==0"/>
              <span v-else>--</span>
            </td>
          </tr>
          <tr v-else>
      ...
      <div class="data-footer">
        <div class="row align-items-center justify-content-between">
          ...
          <div class="col-auto">
            <Bootstrap5Pagination :data="pagination" :limit="5"  @pagination-change-page="getVouchers" />
          </div>
        </div>
      </div>
    </div>
...
async function getVouchers(page = 1){
...
function viewVoucher(slack){
  router.push('/voucher/'+slack);
}
function editVoucher(slack){
  router.push('/voucher/edit/'+slack);
}
