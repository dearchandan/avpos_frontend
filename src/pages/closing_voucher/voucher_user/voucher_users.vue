    ...
    <div class="content">
      <div class="page-heading">
        ...
        <div class="row align-items-center justify-content-between mb-3">
          <div class="col-auto">
            <h2 class="mb-0">{{ $t('Voucher Users') }}</h2>
          </div>
          <div class="col-auto">
            <router-link class="btn btn-sm btn-light-primary2" to="/voucher_user/add" >+ {{ $t('New Voucher User') }}</router-link>
          </div>
        </div>
      ...
      <div v-else class="table-responsive">
        <table class="table table-list-data table-hover">
          ...
          <tbody>
          <tr v-if="voucher_users != ''" v-for="(voucher_user,branch_index) in voucher_users" :key="branch_index" >
            <td role="button">{{ voucher_user.name }}</td>
            <td role="button">{{ voucher_user.iqama_or_passport_number }}</td>
            <td role="button">{{ voucher_user.country }}</td>
            <td role="button">{{ voucher_user.created_at_label }}</td>
            <td role="button">
              <div class="form-feild">
                <div class="form-check form-switch form-switch-2">
                  <input class="form-check-input" type="checkbox" role="switch" id="Status_id" @change="changeStatus(voucher_user.slack)" :checked="voucher_user.status == 1">
                </div>
              </div>
            </td>
            <td class="text-end">
              <ActionButtonGroup :permissions="permissions" @edit="editVoucherUser" :slack="voucher_user.slack" />
            </td>
          </tr>
          <tr v-else>
      ...
      <div class="data-footer">
        <div class="row align-items-center justify-content-between">
          ...
          <div class="col-auto">
            <Bootstrap5Pagination :data="pagination" :limit="5"  @pagination-change-page="getVoucherUsers" />
          </div>
        </div>
      </div>
    </div>
...
const voucher_users = ref([]);
const pagination = ref([]);

async function getVoucherUsers(page = 1){
  ...
  await axios.get('/api/voucher_users', { params : {
    ...
    voucher_users.value = response.data.data;
...
//End filter code
function editVoucherUser(slack){
  router.push('/voucher_user/edit/'+slack);
}
