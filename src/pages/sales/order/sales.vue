...
onMounted(() => {
  checkPermissions();
  getTerminals();
  if(route.query.order_filters != '' && route.query.order_filters != undefined){
    Object.assign(filters, JSON.parse(route.query.order_filters));
  }else{
    getOrders();
  }
  getZatca();
...
function viewOrder(slack) {
  router.push({ name: 'View Order',params:{slack}, query: { order_filters: JSON.stringify(filters) } });
}
