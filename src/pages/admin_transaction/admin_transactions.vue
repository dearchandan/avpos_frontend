        ...
        <template v-slot:subheader>
            <div class="header-nav-wrap">
                <div class="row align-items-center">
                    ...
                    <div class="col-auto">
                        <h5 class="fw-bold">{{ $t('Transactions') }}</h5>
                    </div>
                </div>
            </div>
        </template>
            ...
            <div class="page-heading">
                ...
                <div class="row align-items-center justify-content-between mb-3">
                  ...
                  <div class="col-auto" v-if="permissions.write">
                    <router-link class="btn btn-sm btn-light-primary2" to="/admin/transaction/add" >+ {{ $t("New Transaction") }}</router-link>
                  </div>
                </div>
                            ...
                            <div class="col-6 col-md-auto d-md-block">
                              <div class="form-field">
                                ...
                                <select class="form-control form-control-sm form-control-solid"
                                  ...
                                  <option value="">{{ $t("All") }}</option>
                                  <option value="1">{{ $t("SUBSCRIPTION_PURCHASE") }}</option>
                                  <option value="2">{{ $t("ADDON_PURCHASE") }}</option>
                                  <option value="3">{{ $t("SUBSCRIPTION_RENEWAL") }}</option>
                                  <option value="4">{{ $t("ADDON_RENEWAL") }}</option>
                                </select>
                              </div>
                            </div>
                            <div class="col-6 col-md-auto d-md-block">
...
async function getTransactions(page = 1){
    ...
    await axios.get('/api/admin_transactions', { params : {
            page : page,
...
function viewTransaction(slack){
    router.push('/admin/transaction/'+slack);
}
...
async function deleteTransaction(slack){
  ...
  if(is_confirmed){
    ...
    await axios.post('/api/admin_transaction/delete',{
      slack : slack
    }).then( (response) => {
