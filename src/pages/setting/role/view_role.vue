<template>
  <DefaultLayout>
    ...
    <div class="content role-checkbox-content" v-if="is_loaded">
...
function menu_checker(payload) {
  if (payload.event.target.checked == true) {
    if (payload.type == "ACTIONS") {
      ...
      menu_selected.value = array_remove(menu_selected.value, payload.parent);
      menu_selected.value.push(payload.parent);

      menu_selected.value = array_remove(menu_selected.value, payload.grand_parent);
